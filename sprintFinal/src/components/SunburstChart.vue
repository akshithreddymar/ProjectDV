  ...
  setup(props, { emit }) {
    ...
    const processData = async () => {
      try {
        ...
        const { selectedState, yearStart, yearEnd, selectedCarrier } = props.filters || {}
    ...
    const navigateTo = (crumb) => {
      ...
      // zoom behavior could go here later
    }
    ...
    const drawSunburst = async (animate = true) => {
      try {
        ...
        const width = Math.min(container?.clientWidth || 450, 450)
        const height = width
    ...
    // 🔹 Redraw on state / year / airline changes
    watch(
      () => [props.filters.selectedState, props.filters.yearStart, props.filters.yearEnd, props.filters.selectedCarrier],
      () => drawSunburst(false)
    )
