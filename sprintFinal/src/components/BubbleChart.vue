  ...
  setup(props, { emit }) {
    ...
    const processData = async () => {
      try {
        ...
        const { yearStart, yearEnd } = props.filters || {}
    ...
    const drawChart = async (animate = true) => {
      try {
        ...
        // Always recompute so year slider is respected
        ...
        // Size Scale: Square root for total flights
        ...
        const labelData = data.slice(0, 8)
    ...
    // Watch for ANY filter changes (state, carrier, year range, airport)
    watch(
      () => props.filters,
      () => {
        drawChart(false)
